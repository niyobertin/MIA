...
/** Pure JS SHA-256 so hashing is identical on all platforms (no SubtleCrypto drift). */
...
export async function hashPassword(password: string): Promise<string> {
  ...
  const digest = sha256Sync(`${salt}:${password}`);
  return `${salt}:${digest}`;
}

export async function verifyPassword(password: string, storedHash: string): Promise<boolean> {
  if (!storedHash || storedHash === 'legacy:unmigrated') {
    return false;
  }
  ...
  const candidate = sha256Sync(`${salt}:${password}`);
